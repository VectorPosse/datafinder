...
visualize_dataframe <- function(dataframe, override = FALSE) {
    ...
    if (nrow(dataframe) > 10000){
        ...
        warning("This data set has a lot of rows and you may experience some perfomance
            issues.")
    }
    ...
    if (override) {
        ...
        if (ncol(dataframe) > 10) {
            ...
            warning("This data set has a lot of columns so we will only take the first
                10 so that the plot is still legible.")
